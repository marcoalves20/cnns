...
# ================ Model training ===================
Cnn = Cnn_architecture(architecture='ResNet50', input_shape=[224,224], nclasses=nclasses)
Cnn.create()
...
H = Cnn.model.fit(batch_generator(train_generator), validation_data=batch_generator(validation_generator),
                        epochs=10, steps_per_epoch=train_generator.__len__(), verbose=1,
                        validation_steps=validation_generator.__len__())
# Cnn.model.trainable = True
# opt = Adam(lr=1e-5)
# Cnn.model.compile(loss=losses, optimizer=opt, metrics=["accuracy"], loss_weights=lossWeights)
# H = Cnn.model.fit(batch_generator(train_generator), validation_data=batch_generator(validation_generator),
#                         epochs=6, steps_per_epoch=train_generator.__len__(), verbose=1,
#                         validation_steps=validation_generator.__len__())
